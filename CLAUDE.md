...
| 문서 | 역할 | 작성 시점 |
|------|------|---------|
| `plans/plan-xx.md` | 구현 계획/체크리스트 | **모든 구현/리팩토링 시작 전 필수** |
| `HISTORY.md` | 과정 (Log) | 실시간 (매 작업 직후) |
| `adr/*.md` | 근거 (Decision) | 결정 시 (작업 전/중) |
| `SUMMARY.md` | 결과 (Outcome) | **Phase 종료 시 필수** |

---

## 작업 워크플로우

> ⚠️ **필수 규칙**: 파일 생성, 코드 리팩토링, 설정 변경, **버그 수정** 등 **모든 작업에 앞서 반드시 `plans/plan-xx.md`를 먼저 작성**하고 사용자 승인을 받은 후 진행한다. (작업 로그 역할)

1. **Plan 작성** → 사용자 승인 요청 (커밋하지 않음)
2. **승인 후** → 구현 진행
3. **작업 항목 완료 시 `plans/plan-xx.md`에 체크(`[x]`)를 표시**
4. **Plan 완료 시 상태를 `✅ 완료`로 업데이트**하고 완료일을 기록
5. **작업 완료 후 Plan의 `[ ]` 체크리스트를 `[x]`로 업데이트** (구현 즉시 체크)
6. **작업 완료 후 `HISTORY.md`를 업데이트**
7. **구현 + HISTORY 업데이트 완료 후 커밋 & 푸시**
8. **Phase 완료 시**:
   - `ROADMAP.md`를 업데이트 (✅ 완료 표시, 타임라인 갱신)
   - **`SUMMARY.md`를 작성**하여 해당 Phase의 최종 변경 사항을 정리
9. **커밋 전 `.gitignore` 파일을 확인**
10. **`CLAUDE.md` 수정 시 즉시 커밋 & 푸시** (예외: 프로젝트 가이드라인 변경은 즉시 반영)
