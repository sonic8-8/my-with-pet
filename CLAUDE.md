...
4. 작업 완료 후 `HISTORY.md`를 업데이트한다.
5. **커밋 전 `.gitignore` 파일을 확인**하여 민감 파일(`.env`, `gcp-key.json` 등)이 staging에 포함되지 않도록 한다.
6. **`CLAUDE.md` 수정 시 즉시 커밋 & 푸시**한다. (프로젝트 가이드라인 변경은 즉시 반영)
7. **큰 단위 작업 완료 시 커밋 & 푸시**를 수행한다.
